...
import 'package:newamariders/general/returnedDate.dart';
...
class ReturnedOrders extends StatefulWidget {
  var selectedDate;
  var end;
  ReturnedOrders({super.key, this.restorationId});
  final String? restorationId;

  @override
  State<ReturnedOrders> createState() =>
      _ReturnedOrdersState();
}

class _ReturnedOrdersState extends State<ReturnedOrders>with RestorationMixin {
            ...
            children: [
              ...
              Visibility(
                ...
                child: ListView.builder(
                  ...
                  shrinkWrap: true,
                  itemCount: myReturnedOrders?.length,
                  itemBuilder: (context, index){
                    var dt3 = DateTime.fromMillisecondsSinceEpoch(
                          int.parse(myReturnedOrders![index]['assignTime']));
                      var TAS3 = DateFormat('dd/MM/yyyy').format(dt3);
                    return Container(
                          margin: const EdgeInsets.all(10),
                          padding: EdgeInsets.all(15),
                          decoration: BoxDecoration(
                              color: myReturnedOrders![index]['status'] == 'Pending'
                                  ? Color.fromARGB(255, 255, 17, 0)
                                  : myReturnedOrders![index]['status'] == 'Processing'
                                      ? Colors.blue
                                      : myReturnedOrders![index]['status'] == 'Transit'
                                          ? const Color.fromARGB(
                                              255, 102, 92, 0)
                                          : myReturnedOrders![index]['status'] == 'Delivered'
                                              ? Colors.green
                                  ...
                                  Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                          builder: (context) => Order(
                                              orderno: myReturnedOrders![index]['orderID'],
                                              outlet: myReturnedOrders![index]['outlet'],
                                              status: myReturnedOrders![index]['status'])));
                                },
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(
                                      'Order No. ${myReturnedOrders![index]['orderID']}',
                                    ...
                                    Text('outlet:${myReturnedOrders![index]['outlet']}',
                                        style: const TextStyle(
                                            fontSize: 15, color: Colors.white)),
                                    Text('Status:${myReturnedOrders![index]['status']}',
                                        style: const TextStyle(
                                            fontSize: 15, color: Colors.white)),
                                    const SizedBox(
                                      height: 10,
